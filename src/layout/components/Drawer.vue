<template>
  <v-navigation-drawer :right="$vuetify.rtl" :value="$store.state.drawer" app>
    <v-list dense>
      <v-list-item>
    ...
    <v-list dense>
      <template v-for="item in items">
        <template v-if="item.children.length === 0">
          <v-list-item
            v-bind:style="[
              item.route === $store.state.active_nav
                ? {
                    background:
                      $vuetify.theme.themes[
                        $store.state.dark ? 'dark' : 'light'
                      ].primary,
                    color: 'white',
                  }
                : {},
        ...
        <template v-else>
          <v-list-group class="sub-group" :prepend-icon="item.icon">
            ...
            <v-list-item
              v-bind:style="[
                child.route === $store.state.active_nav
                  ? {
                      background:
                        $vuetify.theme.themes[
                          $store.state.dark ? 'dark' : 'light'
